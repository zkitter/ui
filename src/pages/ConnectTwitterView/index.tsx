...
export default function ConnectTwitterView(props: Props): ReactElement {
  ...
  switch (viewType) {
    ...
    case ViewType.verify:
      content = (
        <VerifyView setViewType={setViewType} twitterAuth={twitterAuth} onResetAuth={onResetAuth} />
      );
      break;
    case ViewType.done:
      content = <DoneView />;
...
function WelcomeView(props: { setViewType: (v: ViewType) => void }): ReactElement {
  ...
  return (
    <div className="flex flex-col flex-nowrap flex-grow my-4 mx-8 signup__content signup__welcome">
      ...
      <div className="my-4">
        Connect your Twitter account to mirror your post to your Twitter feed, and allow Autism
        users to discover your profile by your Twitter handle.
      </div>
      <div className="flex-grow flex flex-row mt-8 flex-nowrap items-end justify-center">
        <Button btnType="primary" onClick={connectTwitter}>
          Next
        </Button>
      </div>
    </div>
...
function VerifyView(props: {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (user?.joinedTx) {
        const verified = await verifyTweet(
          user?.address,
      ...
      if (twitterAuth?.username) {
        const resp = await fetch(
          `${config.indexerAPI}/twitter/check?username=${twitterAuth?.username}`
        );
        const json = await resp.json();
    ...
    try {
      ...
      if (!json?.error && json?.payload) {
        const [twitterHandle, _, tweetId] = json.payload
          ...
          .split('/');
        await dispatch(
          submitProfile(ProfileMessageSubType.TwitterVerification, tweetId, twitterHandle)
        );
        dispatch(
          setUser({
            ...user,
    ...
    <div className="flex flex-col flex-nowrap flex-grow my-4 mx-8 signup__content signup__welcome">
      ...
      <div className="my-4">
        <div className="border rounded-xl m-4 p-4 flex flex-row flex-nowrap">
          <img className="rounded-full w-12 h-12" src={twitterAuth?.profileImageUrlHttps} />
      ...
      <div className="flex-grow flex flex-row mt-8 mb-4 flex-nowrap items-center justify-center">
        {selected?.type !== 'gun' ? (
          <div>{`Please first login to Autism before continuing.`}</div>
        ) : (
          <>
            <Button
              btnType="secondary"
            ...
            <Button btnType="primary" onClick={onVerify} loading={posting}>
              Verify
            </Button>
          </>
        )}
...
function DoneView(): ReactElement {
  ...
  return (
    <div className="flex flex-col flex-nowrap flex-grow my-4 mx-8 signup__content signup__welcome">
      ...
      <div className="my-4 text-center">{`You are connected to ${twitterHandle} on Twitter!`}</div>
      <div className="flex-grow flex flex-row mt-8 flex-nowrap items-end justify-center">
