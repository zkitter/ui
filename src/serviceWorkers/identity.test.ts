...
describe('Identity Service', () => {
  ...
  it('should init service', async () => {
    ...
    expect(await identity.getStatus()).toStrictEqual({ unlocked: false, currentIdentity: null });
    expect(pushReduxActionStub.args[0][0]).toStrictEqual({
      type: 'worker/setUnlocked',
      payload: false,
    });
  });
