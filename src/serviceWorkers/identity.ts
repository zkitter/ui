...
export type Identity = GunIdentity | ZKPRIdentity | InterrepIdentity | TazIdentity;
...
export class IdentityService extends GenericService {
    ...
    return new Promise(async resolve => {
      ...
      req.onupgradeneeded = event => {
        ...
        if (event.oldVersion < 3) {
          const kvStore = db.createObjectStore('keyvalue', {
            keyPath: ['id'],
          });
  ...
  wrapIdentity(id: Identity): Identity {
    if (id.type === 'gun') {
      return {
        ...id,
        privateKey: this.passphrase ? decrypt(id.privateKey, this.passphrase) : '',
      };
    }
  ...
  async getIdentities(): Promise<Identity[]> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      request.onsuccess = () => {
        const ids: Identity[] = (request.result || []).map((id: any) => {
          return {
            address: id.address,
            type: id.type,
            nonce: id.nonce,
