...
export class ZKChatClient extends EventEmitter2 {
  ...
  private async _validateConvos(activeChats: { [chatId: string]: InflatedChat }) {
    const validChats: {
  ...
  private async _load() {
    ...
    this.messages = {};
    this.activeChats = Object.entries(bucket.activeChats).reduce(
      (acc: any, [chatId, chat]: any[]) => {
        acc[chatId] = {
          ...chat,
          messages: [],
        };
  ...
  deriveSharedKey = async (chat: Chat) => {
    ...
    if (chat.type === 'DIRECT') {
      sk = deriveSharedKey(chat.receiverECDH, priv);
    }
