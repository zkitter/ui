...
export async function verifyTweet(account?: string, tweetURL?: string, sessionHandle?: string): Promise<boolean> {
    if (!tweetURL || !account) return false;

    let id;
    try {
        const [twitterHandle, _, tweetId] = tweetURL
...
export const getSession = async (selected: Identity | null): Promise<{
    token: string;
    username: string;
    reputation: string;
