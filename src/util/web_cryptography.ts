...
export class WebCryptography {
  ...
  constructor() {
    if (!window) throw 'This module is intended to work only in the browser';
  }
  ...
  public async generateSymmetricKey(): Promise<string> {
    ...
    if (key) {
      const raw: JsonWebKey = await crypto.subtle.exportKey('jwk', key);
   ...
   */
  public async deriveSharedSecretKey(
    ...
    const raw: JsonWebKey = await crypto.subtle.exportKey('jwk', derivedKey);
    return JSON.stringify(raw);
  }
